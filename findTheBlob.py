...
def startMoving(colorFound):
    if colorFound == "false":
        turnBy(40, "deg")
    
    pictureTaken = takePicture()
    pixelReturned = findColorSpot(pictureTaken, 1)
    
    if pixelReturned == 0:
        startMoving("false")
    elif pixelReturned/getWidth(pictureTaken) > 0.45 and pixelReturned/getWidth(pictureTaken) < 0.55:
        forward(1, 1)
        startMoving("true")
    elif pixelReturned/getWidth(pictureTaken) < 0.45:
        turnBy((90 * 0.5) * ((pixelReturned/getWidth(pictureTaken))/0.5), "deg")
        startMoving("true")
    elif pixelReturned/getWidth(pictureTaken) > 0.55:
        turnBy( (-90 * 0.5) * (( (pixelReturned - (getWidth(pictureTaken)/2))/getWidth(pictureTaken))))
        startMoving("true")
    elif pixelReturned == 1:
        print("color found!")
    else
        print("what")
